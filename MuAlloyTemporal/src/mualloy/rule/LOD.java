    ...
    for (int i = 0; i < argSize; i++) {
      ...
      if (argSize == 2) {
        ...
        if (parent instanceof Body) {
          ...
          if (p.getOp() == BinaryOp.IMPLIES || p.getOp() == BinaryOp.IFF || p.getOp().getGroup() == BinaryOp.Group.TEMPORAL_OPERATOR) {
        ...
        //    && ((UnaryFormula) parent).getOp() == UnaryOp.NOT || ((UnaryFormula) parent).getOp().getGroup() == UnaryOp.Group.TEMPORAL_OPERATOR ) {
          ...
          if (AlloyUtil.isValidModel(mutant)) {
        	...
        	String mutatedNodePathAsString = computeNodePathAsString(parent,
                    opt.getPSV());
        	System.out.println(mutatedNodePathAsString);
        	System.out.println(((UnaryFormula)parent).getOp());
          throw new RuntimeException(
