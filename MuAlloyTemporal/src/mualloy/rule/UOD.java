...
public class UOD {
  ...
  /**
   * Delete * (reflexive transitive closure), ^ (transitive closure) and ~ (transpose) and '(prime).
   */
  public static MutationData mutate(UnaryExpr unaryExpr, ModelUnit mu, Opt opt, MInfo mi) {
    if (mi.getType() == MInfo.MType.IGNORE) {
      return null;
    }
    MutationData res = null;
    UnaryExpr.UnaryOp original = unaryExpr.getOp();
    if (original == UnaryExpr.UnaryOp.PRIME || original.getGroup() == UnaryExpr.UnaryOp.Group.CLOSURE || (
...
//            String mutatedPartAsString = parent.accept(opt.getPSV(), null);
   ...
   * Delete ! (not).
   */
  public static MutationData mutate(UnaryFormula unaryFormula, ModelUnit mu, Opt opt, MInfo mi) {
    if (mi.getType() == MInfo.MType.IGNORE) {
      return null;
    }
    MutationData res = null;
...
//            String mutatedPartAsString = parent.accept(opt.getPSV(), null);
    ...
    else if(unaryFormula.getOp().getGroup() == UnaryFormula.UnaryOp.Group.TEMPORAL_OPERATOR){
    	
        UnaryFormula.UnaryOp original_op = unaryFormula.getOp();
     	unaryFormula.setOp(UnaryFormula.UnaryOp.NOOP);
//              String mutatedPartAsString = parent.accept(opt.getPSV(), null);
        String mutatedNodePathAsString = AlloyUtil
        ...
        if (AlloyUtil.isValidModel(mutant)) {
          ...
          switch (mi.getType()) {
            ...
            case FUN:
              PredOrFun mutatedPredOrFun = (PredOrFun) mi.getNode().accept(new CloneVisitor(), null);
           	  unaryFormula.setOp(original_op);
              equivModel = MutantEquivalenceChecker
                  .buildEquivModel(mu, mutatedPredOrFun, opt.getScope());
              break;
            case FACT:
