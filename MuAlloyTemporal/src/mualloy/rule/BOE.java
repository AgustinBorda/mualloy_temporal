...
public class BOE {
  ...
  public static MutationData mutate(BinaryExpr binaryExpr, ModelUnit mu, Opt opt, MInfo mi) {
...
//        String mutatedPartAsString = binaryExpr.accept(opt.getPSV(), null);
  ...
  public static MutationData mutate(BinaryFormula binaryFormula, ModelUnit mu, Opt opt, MInfo mi) {
    if (mi.getType() == MInfo.MType.IGNORE) {
      return null;
    }
    if (binaryFormula.getOp().getGroup() != BinaryFormula.BinaryOp.Group.SET_OPERATION
        ...
        && binaryFormula.getOp().getGroup() != BinaryFormula.BinaryOp.Group.TEMPORAL_OPERATOR
